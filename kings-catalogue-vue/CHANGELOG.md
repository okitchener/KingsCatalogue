...
- Added cart item variant summaries for the newly supported option combinations.

### Changed
- Updated product data so non-clothing items no longer rely on placeholder size/color values like `One Size` or `Default`.
- Updated add-to-cart and product-details modals to render option dropdowns only for attributes present on that product.
- Updated option dropdown behavior to hide selectors when there is only one available choice.
- Updated cart variant matching to use all supported option fields when grouping items.

### Fixed
- Fixed products showing irrelevant dropdowns such as color or size when those attributes do not apply.
- Fixed cart item merging and removal logic so products with different size, color, volume, or scent selections stay distinct.
- Fixed cart item labels so only the selected attributes that actually exist are displayed.
